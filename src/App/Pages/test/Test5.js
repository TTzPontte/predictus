  ...
  return (
      <FormProvider {...methods}>
        <Form onSubmit={methods.handleSubmit(onSubmit)}>
          ...
          {methods.formState.errors.documentNumber && <span>{methods.formState.errors.documentNumber.message}</span>}
          <br />
          <input type="submit" value="Realizar Consulta" /><br /><br /><br />
        </Form>
        <br /><br />
        ...
        {isResultViewVisible ? <button onClick={() => handleBaixarPDF(personType, state3)}>Baixar Relatório PDF</button> : null}<br /><br /><br />
        {isResultView2Visible ? <Row className="w-100">
          <Col className="w-100">
            {state2 && state2.length > 0 && <Results list={state2} pfOuPj="PJ" />}
          </Col>
        </Row> : null}<br />
        {isResultView2Visible ? <button onClick={handleConsultarSocios}>Consultar Sócios</button> : null}<br /><br />
      </FormProvider>
  );
}
